...
def get_percentile_status(percentile, config):
    ...
    if percentile <= levels['extreme_low']:
        return ("极度低估", "当前比价处于历史极低区域，相对配置价值突出。", "强烈超配", 2)
    elif percentile <= levels['low']:
        return ("低估", "当前比价处于历史低位区域，具备较好的相对性价比。", "超配", 1)
    elif percentile < levels['high']:
        return ("中性", "当前比价处于历史中位附近，估值相对均衡。", "标配", 0)
    elif percentile < levels['extreme_high']:
        return ("高估", "当前比价处于历史高位区域，短期继续追高的性价比偏弱。", "减仓", -1)
    else:
        return ("极度高估", "当前比价处于历史极高区域，需警惕估值回归风险。", "不配置", -2)
...
def generate_analysis(analysis_results):
    ...
    for index_code, data in analysis_results.items():
        ...
        conclusions[index_code] = {
            ...
            'percentile': {'value': percentile, 'status': p_status, 'description': p_desc, 'score': p_score},
            'trend': {'status': trend, 'description': t_desc, 'score': t_score, 'changes': {'5d': data.get('change_5d'), '10d': data.get('change_10d'), '20d': data.get('change_20d')}},
            'deviation': {'value': deviation, 'zscore': zscore, 'status': d_status, 'description': d_desc, 'score': d_score},
            'recommendation': {'action': recommendation, 'icon': rec_icon, 'score': round(total_score, 2), 'reasons': [f"历史分位 {percentile:.1f}%：{p_status}", f"近期趋势：{trend}", f"均线偏离 {deviation:+.2f}% / {zscore:+.2f}σ：{d_status}"]},
            'summary': generate_summary(index_name, percentile, p_status, trend, deviation, zscore, d_status, recommendation, rec_icon)
        }
    return conclusions
...
3. 均值回归：偏离度 {deviation:+.2f}% / {zscore:+.2f}σ（{d_status}）
   当前比价{"高于" if deviation > 0 else "低于"}30日均线 {abs(deviation):.2f}%，相对波动位置为 {zscore:+.2f}σ。
