...
- **所有比价对的两端都是可交易指数**——比价只是一个跷跷板信号：哪边便宜就配置哪边
- **沪深300 和恒生指数特殊**——除了参与比价对之外，各自还有独立的**股权溢价指数（ERP）**框架来判断自身绝对估值分位

覆盖中证500/沪深300、中证1000/沪深300、创业板/沪深300、上证50/创业板、300价值/300成长、科创50/上证50、恒生科技/恒生指数共 7 组比价对。通过计算历史分位数、均值回归、趋势判断，输出量化配置建议。
