...
> 自动化分析 A 股与港股主要指数的比价关系——**A 股以沪深300为锚、港股以恒生指数为锚**，一键生成包含智能分析结论的交互式 HTML 报告

## 📊 项目简介

本工具的核心逻辑：

- **所有比价对的两端都是可交易指数**，比价只是一个跷跷板信号：比分位低买分子，分位高买分母
- **沪深300 和恒生指数**在比价系统内与其他分母无异，其特殊性在于比价系统之外各自拥有独立的 **ERP（股权溢价指数）** 框架来判断自身绝对估值分位

覆盖 10 组比价对：中证500/沪深300、中证1000/沪深300、中证1000/中证500、创业板/沪深300、上证50/沪深300、科创50/沪深300、创业板/上证50、300成长/300价值、科创50/上证50、恒生科技/恒生指数。通过计算历史分位数、均值回归、趋势判断，输出量化配置建议。
...
- ✅ **比价系统定时门禁**：主 workflow 仅在工作日 23:18（北京时间）检查数据；仅当交易日且关键 A 股、港股数据齐全时才更新报告与网站
...
- ⚖️ **比价走势图**：主要、特色及外围市场分组展示，包含均线、分位和偏离指标
- 🤖 **智能分析**：详细的分析结论和配置建议
- 🎨 **深色主题**：金融终端风格，专业美观
