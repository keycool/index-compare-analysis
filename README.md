...
本工具按「双市场、双锚点、双框架」架构组织：

- **锚定点交易**：沪深300、恒生指数本身也参与交易，配置决策依据**股权溢价指数（ERP）的历史分位**高低
- **比价交易**：中证500、中证1000、创业板、上证50、300价值、恒生科技等，配置决策依据**相对各自锚定点的比价历史分位**

通过计算历史分位数、均值回归、趋势判断等指标，为投资者提供量化的配置建议。
