...
- 手动运行：GitHub Actions 的 `workflow_dispatch`。
- 代码 `push` 不触发这条生产 workflow；代码变更后如需刷新或恢复网站，请手动运行。
- 自动运行要求上交所交易日、全 A 日线至少 5,000 条、8 个 A 股决策指数以及 `HSI` / `HKTECH` 均已就绪。数据不完整时，最多每隔 10 分钟重试 2 次，仍未就绪则跳过计算和部署。
