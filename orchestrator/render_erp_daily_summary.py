...
def main() -> None:
    ...
    lines.append(
        f"- 当前总框架偏向 `进攻 {pct(erp['aggressive_weight'])} / 防守 {pct(erp['defensive_weight'])}`。"
    )
    lines.append(
        f"- Relative 最新日期 `{relative['date']}`：`500={relative['recommendations']['zz500'] or '标配'}`，`1000={relative['recommendations']['zz1000'] or '标配'}`，`创业板={relative['recommendations']['cyb'] or '标配'}`，`50={relative['recommendations']['sh50'] or '标配'}`。"
    )
    if val300.get("available"):
        lines.append(
            f"- `300价值/成长` 最新日期 `{val300['date']}`，比价 `{val300['ratio']:.4f}`，分位 `{val300['percentile']:.2f}%`，建议 `{val300['recommendation']}`。"
        )
    lines.append("")
    lines.append("## 调仓建议")
    lines.append("")
    for item in positions:
        lines.append(
            f"- `{item['label']}`：当前 `{num(item['current_amount'])}`，目标 `{num(item['target_amount'])}`，建议 `{action_text(item['action'], item['delta_amount'])}`。"
        )
    lines.append("")
    lines.append("## 组合结构")
    lines.append("")
    ...
    lines.append(
        f"- ERP 分位阈值: `low={thresholds['low']}` / `high={thresholds['high']}`"
    )
    lines.append(
        f"- 进攻权重锚点: `low={pct(aggressive_weights['low'])}` / `neutral={pct(aggressive_weights['neutral'])}` / `high={pct(aggressive_weights['high'])}`"
    )
